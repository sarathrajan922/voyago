...
const validationSchema = Yup.object({
  ...
  remember: Yup.boolean().oneOf([true], 'You must agree to the terms and conditions'),
});
...
const PackageDetails: React.FC = () => {
  ...
  const handleSubmit = async(values:any) => {
    ...
    const bookedData = new FormData()
    bookedData.append('FirstName', values.first_name)
    bookedData.append('LastName', values.last_name)
    bookedData.append('Email', values.email)
    bookedData.append('rooms', values?.rooms)
    bookedData.append('packageId',id ?? '' )
    //!replace the userId with logged user id
    bookedData.append('userId', '6498247fee731d64f1c120c3')
    bookedData.append('travelDate',formattedDate)

    console.log(bookedData)

   await bookPackage(bookedData).then(()=>{
    navigate('/payment')
   }).catch((error:any)=>{
    console.log(error.message)
   })
            ...
            <div className="py-5 px-2 max-h-[45rem]">
              <div className="rounded overflow-hidden hover:shadow-lg mt-6 ">
                ...
                <div className="ms-2 px-1">
                  <h2 className="font-semibold mt-3 mb-1">{tourPackage?.packageName ?? 'packageName'}</h2>
                  ...
                  <span className="text-gray-500">{tourPackage?.category ?? 'cateory'}</span>
                  <br />
                  ...
                  <div className="py-3 px-3 text-end  text-lg font-sans text-gray-900 dark:text-white">
                    Starting from <span className="text-gray-500">₹{tourPackage?.price ?? '00.00'}</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      <Form>
        <div className="grid gap-6 mb-6 md:grid-cols-2">
          ...
          <div>
            ...
            />
            <ErrorMessage name="rooms" component="div" className="text-red-500" />
          </div>
        </div>
        ...
        <div className="flex items-start mb-6">
          ...
          <ErrorMessage name="remember" component="div" className="text-red-500" />
        </div>
